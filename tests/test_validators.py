...
def test_validar_data_retorna_tipo_date(data_valida):
    assert isinstance(validar_datas(data_valida), date) == True
...
def test_erro_validar_data(data_invalida):
    ...
    assert f'Data inválida' in str(exc_info.value)

@pytest.mark.parametrize('entrada, tipo_esperado', [
    pytest.param(12, int, id='int(12) -> int'),
    pytest.param(22.6, float, id=f'float(22.6) -> float'),
    pytest.param('103', int, id=f'str(103) -> int'),
    pytest.param('32.6', float, id=f'str(32.6) -> float'),
    pytest.param('32,6', float, id=f'str(32,6) -> float'),
])
def test_validar_numerico_retorna_tipo_numerico(entrada, tipo_esperado):
    ...
    assert type(_entrada) != str
    assert type(_entrada) == tipo_esperado
...
def test_erro_validar_numerico(entrada):
    ...
    assert f'Valor não é um número válido' in str(exc_info.value)
